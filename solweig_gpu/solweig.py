...
def Kside_veg_v2022a(radI, radD, radG, shadow, svfS, svfW, svfN, svfE, svfEveg, svfSveg, svfWveg, svfNveg,
    ...
    if cyl == 1:
        ...
        # Upstream UMEP (SOLWEIG/SOLWEIGpython/Kside_veg_v2022a.py,
        # "### Kside with weights ###") selects per cardinal direction with a
        # plain scalar if/else on azimuth; the GPU port originally rewrote
        # that as torch.where, but azimuth/t are Python scalars here, so the
        # combined condition is a Python bool that torch.where rejects
        # (TypeError). The branch was dead while cyl was hardcoded True.
        # if/else semantics restored verbatim; the zero side keeps the
        # port's grid-shaped zeros (numerically identical to upstream's
        # scalar 0 under the later sums).
        if azimuth > (360 - t) or azimuth <= (180 - t):
            KeastI = radI * shadow * torch.cos(altitude * deg2rad) * torch.sin(aziE * deg2rad)
        else:
            KeastI = torch.zeros((rows, cols), device=device)
        if azimuth > (90 - t) and azimuth <= (270 - t):
            KsouthI = radI * shadow * torch.cos(altitude * deg2rad) * torch.sin(aziS * deg2rad)
        else:
            KsouthI = torch.zeros((rows, cols), device=device)
        if azimuth > (180 - t) and azimuth <= (360 - t):
            KwestI = radI * shadow * torch.cos(altitude * deg2rad) * torch.sin(aziW * deg2rad)
        else:
            KwestI = torch.zeros((rows, cols), device=device)
        if azimuth <= (90 - t) or azimuth > (270 - t):
            KnorthI = radI * shadow * torch.cos(altitude * deg2rad) * torch.sin(aziN * deg2rad)
        else:
            KnorthI = torch.zeros((rows, cols), device=device)
        KsideI = shadow * 0
    ...
    # GPU-port memory artifact: upstream Kside_veg_v2022a.py has no del
    # block at all. temp_vegsh/temp_vbsh/temp_sh are bound only inside the
    # anisotropic-diffuse loops above, so this del must stay gated on that
    # path -- with anisotropic_diffuse == 0 the isotropic branch never binds
    # them and an ungated del dies with UnboundLocalError (dead while
    # anisotropic_sky was hardcoded 1).
    if anisotropic_diffuse == 1:
...
def define_patch_characteristics(solar_altitude, solar_azimuth,
    ...
    # The patch masks below are pure functions of the (timestep-invariant)
    # shadow cubes; a caller that evaluates many timesteps over the same
    # window may precompute them once (identical bits) and pass them in.
    mask_sky, mask_veg, mask_sun, mask_refl = (
        sky_masks if sky_masks is not None else (None, None, None, None)
    )

    for idx in range(patch_altitude.shape[0]):
        # Calculations for patches on sky, shmat = 1 = sky is visible
        temp_sky = (
            mask_sky[idx] if mask_sky is not None
            else ((shmat[:, :, idx] == 1) & (vegshmat[:, :, idx] == 1))
        )
        ...
        temp_vegsh = (
            mask_veg[idx] if mask_veg is not None
            else ((vegshmat[:, :, idx] == 0) | (vbshvegshmat[:, :, idx] == 0))
        )
        ...
        if mask_sun is not None:
            temp_vbsh = None
            temp_sh = mask_sun[idx]
        else:
            temp_vbsh = (1 - shmat[:, :, idx]) * vbshvegshmat[:, :, idx]
            temp_sh = (temp_vbsh == 1)
    ...
    for idx in range(patch_altitude.shape[0]):
        temp_sh = (
            mask_refl[idx] if mask_refl is not None
            else ((shmat[:, :, idx] == 0) | (vegshmat[:, :, idx] == 0) | (vbshvegshmat[:, :, idx] == 0))
        )
    ...
    del temp_vegsh, vegetation_surface, temp_sh
...
def Solweig_2022a_calc(i, dsm, scale, rows, cols, svf, svfN, svfW, svfE, svfS, svfveg, svfNveg, svfEveg, svfSveg,
                       ...
                       *, precomputed_shadows=None, out_slice=None, sky_masks=None):
    """
    ...
    if altitude > 0: # # # # # # DAYTIME # # # # # #
        ...
        # Shadow  images
        if usevegdem == 1:
            if precomputed_shadows is None:
                ...
                # Windowed callers run the march over the (larger) read
                # window themselves and hand back the read-window tensors;
                # values inside out_slice are bit-identical to running the
                # march here because the march is a per-output-cell ray
                # trace with no cross-output coupling. The shadow/wall
                # fields stay read-sized until the ground-view-factor walk
                # below has consumed them, then everything is cropped.
                vegsh, sh, wallsh, wallsun, wallshve, facesun = precomputed_shadows
            shadow = sh - (1 - vegsh) * (1 - psi)
        else:
            if precomputed_shadows is not None:
                raise ValueError("precomputed_shadows requires usevegdem == 1")
        ...
        # # # # Ground View Factors # # # #
        # gvf_2018a -> sunonsurface_2018a walks up to `second` pixels along
        # each search azimuth, so its inputs must keep the read-window
        # extent even when every other computation runs write-cropped.
        # dsm always carries the read extent (the march needs it too).
        rr, cc = dsm.shape
        ...
        gvfalbnoshW, gvfLupN, gvfalbN, gvfalbnoshN, gvfSum, gvfNorm = gvf_2018a(wallsun, walls, buildings, scale, shadow, first,
                second, dirwalls, Tg, Tgwall, Ta, emis_grid, ewall, alb_grid, SBC, albedo_b, rr, cc,
                                                                 Twater, lc_grid, landcover)

        if out_slice is not None:
            # Everything past the GVF walk is per-cell (elementwise): crop
            # here and the write-window values stay bit-identical.
            gvfLup, gvfalb, gvfalbnosh, gvfLupE, gvfalbE, gvfalbnoshE, \
            gvfLupS, gvfalbS, gvfalbnoshS, gvfLupW, gvfalbW, gvfalbnoshW, \
            gvfLupN, gvfalbN, gvfalbnoshN, gvfSum, gvfNorm = (
                t[out_slice] for t in (
                    gvfLup, gvfalb, gvfalbnosh, gvfLupE, gvfalbE,
                    gvfalbnoshE, gvfLupS, gvfalbS, gvfalbnoshS, gvfLupW,
                    gvfalbW, gvfalbnoshW, gvfLupN, gvfalbN, gvfalbnoshN,
                    gvfSum, gvfNorm)
            ...
            Tg = Tg[out_slice]
            if Tgwall.shape == (rr, cc):
                # A real per-cell wall-temperature field crops with the
                # window; a scalar or (1, 1) class-property wrapper must
                # pass through untouched (it broadcasts either way).
                Tgwall = Tgwall[out_slice]
            shadow = shadow[out_slice]
        ...
        if out_slice is not None:
            emis_grid = emis_grid[out_slice]
            if landcover == 1:
                lc_grid = lc_grid[out_slice]
    ...
    if anisotropic_sky == 1:
        ...
        Ldown, Lside, Least_, Lwest_, Lnorth_, Lsouth_ \
                  = Lcyl_v2022a(esky, L_patches, Ta, Tgwall, ewall, Lup, shmat, vegshmat, vbshvegshmat,
                                altitude, azimuth, rows, cols, asvf, sky_masks=sky_masks)
